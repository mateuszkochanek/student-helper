...
class RulesForm(Form):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields['tresh_type'] = ChoiceField(choices=TYPES)
        self.fields['tresh_type'].label = '{0:d}. Wybierz typ w jakim jest skala, według której wystawiana jest ocena końcowa:'.format(i)
        i += 1
        mark = 2
        half = 0
        j = 0
        while mark+half < 6:
            name_p = 'p_{0:d}_{1:d}'.format(mark, half*10)
            name_k = 'k_{0:d}_{1:d}'.format(mark, half*10)
            if mark != 2 and half != 0:
                self.fields[name_p] = FloatField(required=True)
                self.fields[name_p].label = 'Początek zakresu dla oceny {0:f}'.format(mark+half)
            if mark != 5 and half != 0.5:
                self.fields[name_k] = FloatField(required=True)
                self.fields[name_k].label = 'Koniec zakresu dla oceny {0:f}'.format(mark + half)
            if j%2 == 0:
                half += 0.5
            else:
                mark += 1
    ...
    def save(self):
        ...
        #    Components.objects.add_record(self.course_id, enum_form[form], self.cleaned_data['{}_w'.format(form)],
